...
$(document).ready(function() {
	...
	$('#solution_param').change(function () {//updates the components
		populateComponentList();
		SubmitEnableOrDisable();
	});
	
	$('#component_param').change( function() {//updates the years
	...
	$('#monthparam_start').change(function() { //this will update the days
	...
	$('#dayparam_end').click(function() {//Once user clicks the day, check to see if we should enable Submit button
		SubmitEnableOrDisable();
	});
	...
	function checkToSee() { //this checks to see if all the fields(options) have been chosen, and IF SO, will show google chart, nothing, or an alert message
		if (FormIsFilled()) { //this calls a helper function to see if all the fields have been chosen
			if (CompareDate()) { //we will check to make sure start date is earlier than end date, if so we draw the chart, if not we display an overlay with notification 
	...
	$('#Submit7').click(function() {
		...
		var solution = $('#solution_param').val();//Grab selection fields
		...
		var end_date = day_end + "-" + month_end + "-" + year_end; //Makes the day look like 24-JUL-15
		...
		console.log("Report 3 rest call is:" + temp_url);
		...
		$.ajax({
			...
			success: function(response) {
				drawchart(response); // calling helper function
			},
			error: function() {
...
function drawchart(dataValues) {
	...
	var start_date = start_month + "-" + start_day + "-" + start_year;//using the selection fields, create start date
	var end_date = end_month + "-" + end_day + "-" + end_year; //using the selection fields, create start date
	
	var total = 'Solution: ' + solution + ', Component:' + component + ", Errors & Request Counts (" + start_date + " to " + end_date + ")";//will be used in title field below
...
function populateComponentList() {
	...
	var temp_url = replace_url + "GenerateCompListForReport3/" + $sp.val();//you will want to add the solution as the parameter to the url
...
function update_year_and_clear_month(year, month, day, loading) { //this is helper function for when component param changes, update months and year
	...
	var temp_url = replace_url + "GenerateYearListForReport3"; //generate URL to REST call
	temp_url += "/";
	temp_url += solution1;
	temp_url += "/";
	temp_url += compo;
...
function update_days(day_element, month_element, year_element, loading_pic){
	...
	var temp_url = replace_url + "GenerateDays3/" + year + "/" + month;
...
function update_month(month_element, year_element, day_element, loading_pic) {
	...
	var temp_url = replace_url + "GenerateMonthList3/"; //create url to rest service
	temp_url += solution + "/";
	temp_url += comp + "/";
	temp_url += year;
...
function SubmitEnableOrDisable(){//calls helper function "FormIsFilled()" to see whether or not to
	if(FormIsFilled()){
